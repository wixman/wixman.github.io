...
void main()
{
    ...
    vec3 light = normalize(vec3(1.0, 1.0, 0.1)); // light vector
	/*light = normalize(vec3(normalize((u_source.xy) - (u_resolution.xy/2.0)), 0.2));*/


	vec3 i = vec3(0.0,0.0,1.0); // viewing vector

    float shading = clamp(dot(normal,light),0.5,1.0);
    vec3 finalshading = pow(shading, 2.0) * u_mcolor *vec3(1.5); 

    float spec = pow(clamp(dot(light,reflect(vec3(0.0,0.0,-1.0),normal)),0.0,1.0), 3.0);
	vec3 finalspec = spec * value * vec3(3.0);

	float shadowdist = 2.0;
	float shadowmask = 1.0 - tex(v_uv + light.xy/u_resolution * shadowdist);
	shadowmask = pow(shadowmask, 8.5);
	vec3 shadow = mix(u_mcolor * 0.5, u_bgcolor, shadowmask * 1.0);

	float bgmask = fit(value, 0.05, 0.1, 0.0, 1.0); 
	float ssmask = (1.0 - fit(value, 0.01, 0.2, 0.0, 1.0)) * bgmask; 

	vec3 ss = ssmask * u_mcolor * 1.0; // fake sub surface

	vec3 finalcol = mix(shadow, u_mcolor * 0.3, bgmask) + finalshading * bgmask + finalspec + ss ; 
